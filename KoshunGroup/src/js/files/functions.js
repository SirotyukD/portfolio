...
export function tabs() {
	...
	const autoplayTabs = document.querySelectorAll('[data-tabs-auto]');
	...
	if (autoplayTabs.length > 0) {
		...
		autoplayTabs.forEach((tabsBlock, index) => {
			...
			// let activetapProgress = setInterval(changeCounter, 10);
			...
			function changeCounter() {
				...
				counter++;
				progress = Math.round(counter / 3);
				console.log(progress);

				// showLineLoad.style.width = `${progress}%`;

				if (counter === 300) {
					// clearInterval(activetapProgress);
					progress = 0;
					counter = 0;
					// i++;
			...
			// let activetapProgress = setInterval(() => {
			...
			function changeSpanProgress() {

				let showLineLoad = tabsBlock.querySelector('._tab-active span');
				showLineLoad.style.width = `${progress}%`;

				// tabsBlock.addEventListener("click", () => {
			...
			function changeActiveTab() {
				...
				if (i >= tabsTitles.length - 1) {

					tabsTitles[i].classList.remove('_tab-active');

					i = 0;

					tabsTitles[i].classList.add('_tab-active');
					// setValueProgress()
	...
	// 		tabsBlock.addEventListener("click", handllerChangeActiveTab);
	...
	// 		function handllerChangeActiveTab(e) {

	// 			const el = e.target;

	// 			const titlesBlock = e.target.closest('[data-tabs-titles]');
	...
	// 			// if (titlesBlock) {
	// 			// 	// let titles = titlesBlock.querySelectorAll('.tabs__title');
	// 			// 	// console.log(titles);
